...
contract BondingNOM {
    ...
    function DecToF64(uint256 token) public view returns(int128) {
        return int128(token)*10**(int128(prec) - int128(decimals))
    }

    // Convert Fixed Point 64 to Token UInt256
    function f64ToDec(int128 fixed64) public view returns(uin256) {
    ...
    function supplyAtPrice(uint256 price) return(uint256) {
        return f64toDec(
                abdk64.sqrt(
                    abdk64.fromUINT(price >> (uint256(prec) - uint256(decimals))
                )
            )*fromUINT(a));
    }
